...
namespace AdventOfCode2022
{
    ...
    class Day19 : Problem
    {
        protected readonly record struct Blueprint(
            int oreBotOreCost, 
            int clayBotOreCost, 
        ...
        protected List<Blueprint> allBluePrints = new();
        ...
        {
            int totalQuality = 0;
            foreach (var (blueprint, i) in allBluePrints.Select((blueprint, i) => ( blueprint, i ))) {
        ...
        {
            int totalQuality = 1;
            foreach (var (blueprint, i) in allBluePrints.Take(3).Select((blueprint, i) => ( blueprint, i ))) {
                totalQuality *= RunBluePrint(blueprint, 32);
            }
            return totalQuality.ToString();
        }

        protected int RunBluePrint(Blueprint blueprint, int duration = 24)
        {
            Console.WriteLine(string.Format("Running {0}", blueprint));
            ...
            for (int time = 1; time <= duration; time ++) {
                foreach (var state in allStates.ToList()) {
                    ...
                    foreach (var func in buildableBots) { // bots get built (in new states)
            ...
            Console.WriteLine("\tBest state " + StateToString(bestState));
            return bestState.resources.geodes;
        }

        protected void BuildOreBot(ref ((int ore, int, int, int) resources, (int oreBot, int, int, int) botCounts) state, Blueprint blueprint)
        {
            state.resources.ore -= blueprint.oreBotOreCost;
            state.botCounts.oreBot += 1;
        }

        protected void BuildClayBot(ref ((int ore, int, int, int) resources, (int, int clayBot, int, int) botCounts) state, Blueprint blueprint)
        {
            state.resources.ore -= blueprint.clayBotOreCost;
            state.botCounts.clayBot += 1;
        }

        protected void BuildObsidianBot(ref ((int ore, int clay, int, int) resources, (int, int, int obsidianBot, int) botCounts) state, Blueprint blueprint)
        {
            state.resources.ore -= blueprint.obsidianBotOreCost;
            state.resources.clay -= blueprint.obsidianBotClayCost;
        ...
        protected void BuildGeodeBot(ref ((int ore, int, int obsidian, int) resources, (int, int, int, int geodeBot) botCounts) state, Blueprint blueprint)
        {
